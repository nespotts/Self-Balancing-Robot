...
void ApplyRemoteData() {
  ...
  // if (currenttime -setpoint_timer >= setpoint_duration) {
  //   velocity.setpoint = map((float)receive_data.elevator, 0, 1023, -0.35,0.35);
  //   steer.setpoint -= map((float)receive_data.aileron, 0,1023, -control.steer_ang_vel*(setpoint_duration/1000000.0), control.steer_ang_vel*(setpoint_duration/1000000.0));
  //   setpoint_timer = currenttime;
  // }
  cmd_vel.lin_vel = receive_data.elevator;
  cmd_vel.ang_vel = receive_data.aileron;
}
...
void Receive_Data() {
  ...
  if (radio.available()) {
    ...
    // Serial.print(receive_data.balance_adjust);
    // Serial.print("\t");
    // Serial.print(receive_data.autoangularvelocity);
    // Serial.print("\t");
    // Serial.print(receive_data.autoforwardvelocity);
    // Serial.print("\t");
    // Serial.print(receive_data.rotation_adjust);
    // Serial.print("\t");
